 ...
 */
function scrollTo(offset: number, contentHeight: number, viewportHeight: number): NativeSyntheticEvent<NativeScrollEvent> {
  return {
    nativeEvent: {
...
/**
 * The hook without React. `useRef` and `useCallback` outside a component are
 * exactly the identity and storage this needs, and rendering a host component
 * to reach them would put a renderer between the assertion and the rule.
 */
function follower(): { follow: FollowNewest<unknown>; ends: number } {
  const state = { ends: 0 };
  const follow = useFollowNewest<unknown>();
  follow.ref({
    scrollToEnd: () => {
      state.ends += 1;
    },
