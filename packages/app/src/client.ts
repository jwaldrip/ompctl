 ...
 * the daemon, so a phone that loses signal mid-turn must come back to exactly
 * the updates it missed: no gap, no duplicate. The daemon persists every
 * `session/update` with a monotonic per-agent `seq` for precisely this reason,
 * so the client's job is to remember the highest `seq` it has actually
...
export interface BackoffOptions {
   ...
   * never breach the ceiling. Its job is to stop every phone in a household
 ...
 *
 * This is the frame a client with a voice of its own wants: it carries prose
 * already stripped of code fences and paths, and the client speaks it with a
 * local synthesizer. A phone never needs the `speech` PCM below, so audio
 * never has to cross the network for it.
 *
 ...
 *
 * The fallback for a client with no local voice. The daemon only sends this to
 * a device that spoke first, so a client that receives one asked for it, and
 * the honest thing is to play it.
 */
...
export class OmpdClient {
   ...
   * device comes back online or the app returns to the foreground, both of
   * which mean the reason for the wait has probably just gone away.
   *
   ...
   * tab focus would turn a flaky link into a connect loop.
   */
  reconnectNow(): void {
    if (!this.started) return;
  ...
  private openSocket(reason: string): void {
    ...
    socket.onclose = (info) => {
      ...
      if (!this.started) return;
      const cause =
        info.reason && info.reason.length > 0 ? info.reason : `closed (${info.code ?? "no code"})`;
      // A handshake the daemon refused and a cable someone pulled arrive here
      // identically: a browser cannot read the status of a failed websocket
   ...
   * devices, but across successive attempts by one client hammering a daemon
  ...
  private sendAttach(agentId: AgentId): void {
    ...
    const frame: ClientFrame =
      sinceSeq === undefined ? { t: "attach", agentId } : { t: "attach", agentId, sinceSeq };
    this.send(frame);
  }
...
/**
 * Optimistic by design. Web reports connectivity on `navigator.onLine`; React
 * Native has a `navigator` with no such property, and both of those and the
 * absence of `navigator` altogether have to mean "assume reachable", because
 * the failure mode of guessing offline is a client that stops trying.
 *
 * A native build that wants the real answer injects `isOnline` instead.
 */
...
/** `WebSocket` as every target spells it: browsers, and React Native's own. */
 ...
 *
 * This exists because a browser cannot see the status of a websocket upgrade
 ...
 *
 * The endpoint is derived by string surgery rather than through `URL`. React
 * Native's `URL` is getter-only, so `url.protocol = ...` throws under a module's
 * strict mode, and its accessors only recognise `http(s)`, which means a
 * `ws://` socket address does not parse there at all. Both facts turn the one
 * request that can distinguish a dead token from a dead network into a thrown
 * exception, and a probe that always answers `unknown` never stops the retry
