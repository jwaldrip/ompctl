...
import type { DiscoveredAuth, GrantInput } from "./types.ts";
...
export async function beginLogin(opts: BeginLoginOptions): Promise<PendingLogin> {
  ...
  const settled = Promise.withResolvers<GrantInput>();
  // Assigned immediately after the bind below. The handler cannot run before
  // then, and the value it sends to the token endpoint has to be the same
  // string the authorization request carried, byte for byte.
  let redirectUri = "";
   ...
   * what happened.
   */
  function finish(outcome: GrantInput | Error): void {
    if (done) return;
    done = true;
