...
- Two primary-button treatments and two content gutters across pair and connections; unknown metrics render as a bare `--`. Both screens now draw their primary act from `design/PrimaryButton.tsx` (the filled swatch the rest of the app uses), both pad their content by `space.loose`, and an unreported usage reading says "not reported" in place instead of a dash. Gated by `packages/app/test/pair-connections-consistency.test.tsx`. Done 2026-08-23, #116
